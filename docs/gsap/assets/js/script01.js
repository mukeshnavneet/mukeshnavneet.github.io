$(function() {
    ...
    function over() {
        $touch_me_not.addClass("hide-btn");
        TweenMax.to($touch_me_not, 0.5, { x: "-50%", width: WIW, })
        TweenMax.to($touch_me_not_IMG, 0.5, { x: "0%", width: WIW, })
        TweenMax.to($cloud_text, 0.5, { autoAlpha: 1, scale: 1, })
    }

    function out() {
        $touch_me_not.removeClass("hide-btn");
        TweenMax.to($touch_me_not, 0.5, { x: "-50%", width: "200px", })
        TweenMax.to($touch_me_not_IMG, 0.5, { x: "0%", width: "200px", })
        TweenMax.to($cloud_text, 0.5, { autoAlpha: 1, scale: 0 })
    }
    ...
    gsap.to(window, { duration: 1, scrollTo: "#land" });
});
